<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:83:"D:\phpStudy\WWW\class_schedule\public/../application/index\view\user\edit_sche.html";i:1522143659;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<form method="post" action="">
    ...
    <div class="col-md-4">
        <div class="input-group">
            <input name="schedule_name" class="form-control" type="text" value="<?php echo $schedule_name; ?>" placeholder="课表名">
...
//给登录按钮添加点击事件
        $("#save").on('click', function (event) {
            $.ajax({
                type: "POST",
                url: "<?php echo url('edit_save'); ?>",  //设置提交数据处理的脚本文件的地址；
                data: $("form").serialize(),   //将当前表单的数据序列化后提交
                dataType: 'json',              //设置提交数据的类型为json
                success: function (data) {
                    console.log(data);
                    alert(data.message);      //弹出提示验证成功
                    // window.location.href = "<?php echo url('student_index'); ?>"; //允许用户进入后台
